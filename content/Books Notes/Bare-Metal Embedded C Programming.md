...
I2C is a communication protocol invented by Philips Semiconductor.
It is designed for short-distance communication within a single device or between multiple devices on the same board.

#### The key features of I2C

- **A two-wire interface**: uses only two wires, **Serial Data (SDA)** and **Serial Clock (SCL)**, which reduces the number of pins required on the microcontroller.
- **Multi-master and multi-slave**: multiple masters can initiate communication on the bus, and multiple slaves can respond.
    The I2C protocol supports up to 128 devices with 7-bit addressing; the practical limit is 119 due to reserved addresses.
- **Addressable devices**: each device on the I2C bus has a unique address, enabling the master to communicate with specific slaves.
...
- **Serial Data (SDA)**: this line carries the data; it is bidirectional.
- **Serial Clock (SCL)**: this line carries the clock signal generated by the master device.

The two lines are connected to all devices on the bus, with pull-up resistors to ensure that the lines are pulled to a high state when idle.

#### How I2C works

**Master device**: The master device initiates the communication and generate a clock signal, he can address multiple devices.
**Slave device**: The slave device to the master commands and performs the requested operations. Each slave has a unique **7-bit** or **10-bit address** that the master uses to identify it.

##### The communication process

1. **A start condition**: Communication begins with the master generating a start condition. This involves pulling the SDA line low while the SCL line is high.
2. **Address frame**: The master sends the address of the target slave device, followed by a read/write bit indicating the operation type (0 for write 1 for read).
